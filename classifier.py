...
def cnn_model_fn(features, labels, mode):
  ...
  # Input Tensor Shape: [batch_size, 30, 30, 1]
  ...
  # Input Tensor Shape: [batch_size, 15 * 20 * 64]
  # Output Tensor Shape: [batch_size, 1024]
  dense = tf.layers.dense(inputs=pool2_flat, units=2048, activation=tf.nn.relu)

  #dense2 = tf.layers.dense(inputs=dense, units=2048, activation=tf.nn.relu)
...
def classify(argv):
  ...
  for fileid in range(30) :
    sys.stdin = open("batch" + str(fileid) + ".txt", "r")
    lines = sys.stdin.readlines()
    for tempstr in lines :
      tempstr = tempstr.strip()
      if (tempstr == "-1") : continue
      ...
      if (label >= '0' and label <= '9') :
        label = int(label)
      elif (label >= 'A' and label <= 'Z') :
        label = ord(label) - ord('A') + 10
      else :
        label = ord(label) - ord('a') + 10
        
      train_data.append(list(map(int, tempstr[1 : len(tempstr)])))
  ...
  mnist_classifier = tf.estimator.Estimator(
      model_fn=cnn_model_fn, model_dir="C:\\Users\\Tyrion\\Desktop\\submit\\convnet_model")
  ...
  pred = list(mnist_classifier.predict(input_fn=predict_input_fn))
  pred = [p["classes"] for p in pred]

  for i in range(len(pred)) :
    if (pred[i] >= 10) :
      pred[i] = chr(pred[i] + 87)
    else :
      pred[i] = chr(pred[i] + 48)
